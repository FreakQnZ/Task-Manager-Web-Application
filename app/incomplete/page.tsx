...
function Page() {
  ...
  if (isLoading) {
    return (
      <>
      <h1 className='text-3xl pl-10 pt-5 border-0 border-b-2 border-b-success w-fit pb-2 pr-4'>Complete Now!</h1>
  ...
  return (
    <main className='h-full'>
      <h1 className='text-3xl pl-10 pt-5 border-0 border-b-2 border-b-success w-fit pb-2 pr-4'>Complete Now!</h1>
      <div className='m-2 p-2 mt-10 h-5/6 overflow-y-scroll grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1'>
        {users.tasks?.filter((task: any) => task.comp === 0).map((user: any, id: number) => (
          <div key={id}>
